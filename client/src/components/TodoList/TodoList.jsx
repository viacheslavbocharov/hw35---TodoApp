...
function TodoRow({ title, id, deleteTodo, token, setTodos }) {
  ...
  return (
    <li className="todo-row" key={id}>
      ...
      {isEditFieldRequire &&
        <div>
          <input className="field" ref={titleRef} type="text" defaultValue={title} />
        </div>}
      <div className="todo-btns">
        <div>
          {!isEditFieldRequire && <div><input className="btn" type="button" value="Edit" onClick={showEditField} /></div>}
